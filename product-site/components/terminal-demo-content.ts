/**
 * Marketing terminal transcript. `cliVersion` should match the published npm package
 * (e.g. from https://registry.npmjs.org/cistack/latest — same data as npmjs.com/package/cistack).
 */
...
export function buildTerminalDemoLines(cliVersion: string): TerminalDemoLine[] {
  return [
...
export function buildTerminalDemoPlainText(cliVersion: string): string {
  return buildTerminalDemoLines(cliVersion)
    .map((l) => l.text)
    .join("\n");
}
